...
class Invoices extends \PHPUnit_Framework_TestCase
{
    ...
    public function testCrud() { 
        ...
        $id = $onFact->Invoices->add($invoice);  
        $this->assertTrue(is_numeric($id));
        
        $invoiceApi = $onFact->Invoices->view($id);   
        $this->assertArrayContainsArray($invoice, $invoiceApi);
        
        $updatedInvoice = $invoice;
        $updatedInvoice['Invoice']['customer_name'] = 'Jan Van Gyseghem';
        $updatedInvoice['Invoice']['term'] = '30';
        unset($updatedInvoice['Invoiceitem']); // Either update none or update all
         
        $update = $onFact->Invoices->update($id, $updatedInvoice);  
        $this->assertTrue($update);
        
        $invoiceApi = $onFact->Invoices->view($id);    
        $this->assertArrayContainsArray($updatedInvoice, $invoiceApi);
        
        $delete = $onFact->Invoices->delete($id);
